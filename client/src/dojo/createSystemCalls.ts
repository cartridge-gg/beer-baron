...
import { BuyHopsProps, JoinGameProps, GameIdProps, ViewPriceProps, FarmProps, BrewBeerProps, BottleBeerProps, SellBeerProps, CreateGameProps, MakeTradeProps, AcceptTradeProps, CancelTradeProps, ClaimIndulgenceProps, PlaceIndulgencesBidProps } from "./types";
...
export function createSystemCalls(
    ...
    const notify = (message: string, transaction: any) => toast({
        title: transaction.execution_status != 'REVERTED' ? message : transaction.revert_reason,
        variant: 'default'
    });
    ...
    const increment_indulgences_auction = async ({ account, game_id }: ClaimIndulgenceProps) => {
        try {
            const tx = await execute(account, "auctions", "increment_indulgences_auction", [game_id]);
